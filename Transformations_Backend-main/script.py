    ...
    try:
        ...
        for source_name, source_details in data["sources"].items():
            if source_details["source_type"] == "csv":
                data_sources[source_name] = fetch.fetch_data_from_csv(
                ...
                data_sources[source_name] = fetch.fetch_data_from_xml(
                    source_details["location"]
                )
            elif source_details["source_type"] == "excel":
                data_sources[source_name] = fetch.fetch_data_from_excel(
                    source_details["location"]
                )
            elif source_details["source_type"] == "parquet":
                data_sources[source_name] = fetch.fetch_data_from_parquet(
                    source_details["location"]
                )
            elif source_details["source_type"] == "json":
                data_sources[source_name] = fetch.fetch_data_from_json(
                    source_details["location"]
                )
            else:
